...
<?php
			...
			{
					$qry="UPDATE account set bal=0 WHERE name='shirts'";
			...
			$qry="SELECT * FROM trans where toa='workshop' ORDER BY id DESC";
			$result=mysql_query($qry);
			...
			$bal=0;
			$qry="SELECT bal FROM account where name='workshop' ";
			$result=mysql_query($qry);
			while ($row = mysql_fetch_assoc($result))
			{
				$bal=$row['bal'];
